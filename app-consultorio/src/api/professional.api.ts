...
// --- Social Links ---
 createSocialLink: async (data: { type: string; url: string }) => {
  const { data: response } = await api.post("professionals/social-links", data);
  return response;
},

 updateSocialLink : async (id: number, data: { type?: string; url?: string }) => {
  const { data: response } = await api.patch(`professionals/social-links/${id}`, data);
  return response;
},

 deleteSocialLink : async (id: number) => {
  const { data: response } = await api.delete(`professionals/social-links/${id}`);
  return response;
},
...
deleteCertificate : async (id: number) => {
  const { data: response } = await api.delete(`professionals/certificates/${id}`);
  return response;
},
}